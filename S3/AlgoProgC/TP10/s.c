...
//sinon affichage d'erreur
//S: m modifiee (procedure)
void ajouter(float m[NLIG][NCOL],int nb_lig,int nb_col,int i1,float a,int i2)
{

  for(int j=0;j<=nb_col;j++){
    m[i1][j]=m[i1][j]+a*m[i2][j];
  }
                     ...
                     int* nb_piv)
{
    ...
    *nb_piv=0; //initialisation du nombre de pivots
    ...
    while(lig<nb_lig && col<nb_col){
      //Déterminer le coefficient non nul (pivot)
      for(int x=col;x<nb_col;x++){
        if(fabs(s[lig][col])>EPSILON){
          =fabs();
        }
      }

      //S'il n'y a pas de pivot
        //(énoncé)
...
afficher_sys(m,nb_lig,nb_col+1);
dilater(m,nb_lig,nb_col,1,0.10);
afficher_sys(m,nb_lig,nb_col+1);
...
float s5[NLIG][NCOL]={{1,2,0.,0.,0.,3.},{0.,0.,2.,3.,1.,6},{0.,0.,2.,1.,-1.,2.},
{0.,0.,0.,2.,3.,5.},{0,0,2,3,2,7}};
nb_lig=5;
nb_col=6-1;
